document.createElement("canvas").getContext || !function() {
    ...
    function addNamespace(doc, prefix, urn) {
        doc.namespaces[prefix] || doc.namespaces.add(prefix, urn, "#default#VML");
    }
    function addNamespacesAndStylesheet(doc) {
        if (addNamespace(doc, "g_vml_", "urn:schemas-microsoft-com:vml"), addNamespace(doc, "g_o_", "urn:schemas-microsoft-com:office:office"), 
    ...
    function onPropertyChange(e) {
        ...
        switch (e.propertyName) {
          case "width":
            el.getContext().clearRect(), el.style.width = el.attributes.width.nodeValue + "px", 
            el.firstChild.style.width = el.clientWidth + "px";
            break;

          case "height":
            el.getContext().clearRect(), el.style.height = el.attributes.height.nodeValue + "px", 
    ...
    function copyState(o1, o2) {
        ...
        o2.strokeStyle = o1.strokeStyle, o2.globalAlpha = o1.globalAlpha, o2.font = o1.font, 
        o2.textAlign = o1.textAlign, o2.textBaseline = o1.textBaseline, o2.arcScaleX_ = o1.arcScaleX_, 
        o2.arcScaleY_ = o1.arcScaleY_, o2.lineScale_ = o1.lineScale_;
    }
    function getRgbHslContent(styleString) {
        var start = styleString.indexOf("(", 3), end = styleString.indexOf(")", start + 1), parts = styleString.substring(start + 1, end).split(",");
        return (4 != parts.length || "a" != styleString.charAt(3)) && (parts[3] = 1), parts;
    }
    function percent(s) {
    ...
    function hslToRgb(parts) {
        var r, g, b, h, s, l;
        if (h = parseFloat(parts[0]) / 360 % 360, 0 > h && h++, s = clamp(percent(parts[1]), 0, 1), 
        l = clamp(percent(parts[2]), 0, 1), 0 == s) r = g = b = l; else {
            var q = .5 > l ? l * (1 + s) : l + s - l * s, p = 2 * l - q;
            r = hueToRgb(p, q, h + 1 / 3), g = hueToRgb(p, q, h), b = hueToRgb(p, q, h - 1 / 3);
        }
        return "#" + decToHex[Math.floor(255 * r)] + decToHex[Math.floor(255 * g)] + decToHex[Math.floor(255 * b)];
    }
    function hueToRgb(m1, m2, h) {
        return 0 > h && h++, h > 1 && h--, 1 > 6 * h ? m1 + 6 * (m2 - m1) * h : 1 > 2 * h ? m2 : 2 > 3 * h ? m1 + (m2 - m1) * (2 / 3 - h) * 6 : m1;
    }
    function processStyle(styleString) {
        if (styleString in processStyleCache) return processStyleCache[styleString];
        var str, alpha = 1;
        if (styleString = String(styleString), "#" == styleString.charAt(0)) str = styleString; else if (/^rgb/.test(styleString)) {
            for (var n, parts = getRgbHslContent(styleString), str = "#", i = 0; 3 > i; i++) n = -1 != parts[i].indexOf("%") ? Math.floor(255 * percent(parts[i])) : +parts[i], 
            str += decToHex[clamp(n, 0, 255)];
            alpha = +parts[3];
        } else if (/^hsl/.test(styleString)) {
            var parts = getRgbHslContent(styleString);
            str = hslToRgb(parts), alpha = parts[3];
    ...
    function getComputedStyle(style, element) {
        ...
        return "number" == typeof style.size ? computedStyle.size = style.size : -1 != style.size.indexOf("px") ? computedStyle.size = fontSize : -1 != style.size.indexOf("em") ? computedStyle.size = canvasFontSize * fontSize : -1 != style.size.indexOf("%") ? computedStyle.size = canvasFontSize / 100 * fontSize : -1 != style.size.indexOf("pt") ? computedStyle.size = fontSize / .75 : computedStyle.size = canvasFontSize, 
        computedStyle.size *= .981, computedStyle.family = "'" + computedStyle.family.replace(/(\'|\")/g, "").replace(/\s*,\s*/g, "', '") + "'", 
        computedStyle;
    }
    ...
    function CanvasRenderingContext2D_(canvasElement) {
        ...
        this.lineCap = "butt", this.miterLimit = 1 * Z, this.globalAlpha = 1, this.font = "10px sans-serif", 
        this.textAlign = "left", this.textBaseline = "alphabetic", this.canvas = canvasElement;
        var cssText = "width:" + canvasElement.clientWidth + "px;height:" + canvasElement.clientHeight + "px;overflow:hidden;position:absolute", el = canvasElement.ownerDocument.createElement("div");
        el.style.cssText = cssText, canvasElement.appendChild(el);
        var overlayEl = el.cloneNode(!1);
        overlayEl.style.backgroundColor = "red", overlayEl.style.filter = "alpha(opacity=0)", 
        canvasElement.appendChild(overlayEl), this.element_ = el, this.arcScaleX_ = 1, this.arcScaleY_ = 1, 
        this.lineScale_ = 1;
    }
    ...
    function appendStroke(ctx, lineStr) {
        var a = processStyle(ctx.strokeStyle), color = a.color, opacity = a.alpha * ctx.globalAlpha, lineWidth = ctx.lineScale_ * ctx.lineWidth;
        1 > lineWidth && (opacity *= lineWidth), lineStr.push("<g_vml_:stroke", ' opacity="', opacity, '"', ' joinstyle="', ctx.lineJoin, '"', ' miterlimit="', ctx.miterLimit, '"', ' endcap="', processLineCap(ctx.lineCap), '"', ' weight="', lineWidth, 'px"', ' color="', color, '" />');
    }
    function appendFill(ctx, lineStr, min, max) {
        var fillStyle = ctx.fillStyle, arcScaleX = ctx.arcScaleX_, arcScaleY = ctx.arcScaleY_, width = max.x - min.x, height = max.y - min.y;
        if (fillStyle instanceof CanvasGradient_) {
            ...
            if ("gradient" == fillStyle.type_) {
                var x0 = fillStyle.x0_ / arcScaleX, y0 = fillStyle.y0_ / arcScaleY, x1 = fillStyle.x1_ / arcScaleX, y1 = fillStyle.y1_ / arcScaleY, p0 = getCoords(ctx, x0, y0), p1 = getCoords(ctx, x1, y1), dx = p1.x - p0.x, dy = p1.y - p0.y;
                ...
                focus = {
                    x: (p0.x - min.x) / width,
                    y: (p0.y - min.y) / height
                }, width /= arcScaleX * Z, height /= arcScaleY * Z;
            ...
            for (var length = stops.length, color1 = stops[0].color, color2 = stops[length - 1].color, opacity1 = stops[0].alpha * ctx.globalAlpha, opacity2 = stops[length - 1].alpha * ctx.globalAlpha, colors = [], i = 0; length > i; i++) {
                var stop = stops[i];
            ...
            if (width && height) {
                var deltaLeft = -min.x, deltaTop = -min.y;
            ...
            var a = processStyle(ctx.fillStyle), color = a.color, opacity = a.alpha * ctx.globalAlpha;
    ...
    function CanvasPattern_(image, repetition) {
        switch (assertImageIsValid(image), repetition) {
          case "repeat":
          case null:
          case "":
    ...
    function assertImageIsValid(img) {
        img && 1 == img.nodeType && "IMG" == img.tagName || throwException("TYPE_MISMATCH_ERR"), 
        "complete" != img.readyState && throwException("INVALID_STATE_ERR");
    }
    function DOMException_(s) {
        this.code = this[s], this.message = s + ": DOM Exception " + this.code;
    }
    var m = Math, mr = m.round, ms = m.sin, mc = m.cos, abs = m.abs, sqrt = m.sqrt, Z = 10, Z2 = Z / 2, slice = (+navigator.userAgent.match(/MSIE ([\d.]+)?/)[1], 
    ...
    var G_vmlCanvasManager_ = {
        ...
        initElement: function(el) {
            if (!el.getContext) {
                el.getContext = getContext, addNamespacesAndStylesheet(el.ownerDocument), el.innerHTML = "", 
                el.attachEvent("onpropertychange", onPropertyChange), el.attachEvent("onresize", onResize);
    ...
    for (var decToHex = [], i = 0; 16 > i; i++) for (var j = 0; 16 > j; j++) decToHex[16 * i + j] = i.toString(16) + j.toString(16);
    var colorData = {
        aliceblue: "#F0F8FF",
        antiquewhite: "#FAEBD7",
        aquamarine: "#7FFFD4",
        ...
        whitesmoke: "#F5F5F5",
        yellowgreen: "#9ACD32"
    }, processStyleCache = {}, DEFAULT_STYLE = {
        style: "normal",
        variant: "normal",
        weight: "normal",
        size: 10,
        family: "sans-serif"
    }, fontStyleCache = {}, lineCapMap = {
        butt: "flat",
        round: "round"
    ...
    contextPrototype.clearRect = function() {
        ...
        var p = getCoords(this, aX, aY), cp1 = getCoords(this, aCP1x, aCP1y), cp2 = getCoords(this, aCP2x, aCP2y);
        bezierCurveTo(this, cp1, cp2, p);
    }, contextPrototype.quadraticCurveTo = function(aCPx, aCPy, aX, aY) {
        var cp = getCoords(this, aCPx, aCPy), p = getCoords(this, aX, aY), cp1 = {
        ...
        var p = getCoords(this, aX, aY), pStart = getCoords(this, xStart, yStart), pEnd = getCoords(this, xEnd, yEnd);
        this.currentPath_.push({
            type: arcType,
            x: p.x,
            y: p.y,
        ...
        var d = getCoords(this, dx, dy), vmlStr = [], W = 10, H = 10;
        ...
        1 != this.m_[0][0] || this.m_[0][1] || 1 != this.m_[1][1] || this.m_[1][0]) {
            ...
            var max = d, c2 = getCoords(this, dx + dw, dy), c3 = getCoords(this, dx, dy + dh), c4 = getCoords(this, dx + dw, dy + dh);
        ...
        lineStr.push(' ">'), aFill ? appendFill(this, lineStr, min, max) : appendStroke(this, lineStr), 
        ...
        this.aStack_.length && (copyState(this.aStack_.pop(), this), this.m_ = this.mStack_.pop());
        ...
        var m = this.m_, delta = 1e3, left = 0, right = delta, offset = {
            x: 0,
            y: 0
        }, lineStr = [], fontStyle = getComputedStyle(processFontStyle(this.font), this.element_), fontStyleString = buildStyle(fontStyle), elementStyle = this.element_.currentStyle, textAlign = this.textAlign.toLowerCase();
        switch (textAlign) {
          case "left":
          case "center":
          case "right":
          ...
          case "end":
            textAlign = "ltr" == elementStyle.direction ? "right" : "left";
            break;

          case "start":
            textAlign = "rtl" == elementStyle.direction ? "right" : "left";
            break;
        ...
        stroke ? appendStroke(this, lineStr) : appendFill(this, lineStr, {
            x: -left,
            y: 0
        }, {
            x: right,
        ...
        var skewM = m[0][0].toFixed(3) + "," + m[1][0].toFixed(3) + "," + m[0][1].toFixed(3) + "," + m[1][1].toFixed(3) + ",0,0", skewOffset = mr(d.x / Z + 1 - m[0][0]) + "," + mr(d.y / Z - 2 * m[1][0]);
