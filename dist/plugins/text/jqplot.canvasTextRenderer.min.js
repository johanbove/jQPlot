!function($) {
    ...
    $.jqplot.CanvasTextRenderer = function(options) {
        this.fontStyle = "normal", this.fontVariant = "normal", this.fontWeight = "normal", 
        this.fontSize = "10px", this.fontFamily = "sans-serif", this.fontStretch = 1, this.fillStyle = "#666666", 
        this.angle = 0, this.textAlign = "start", this.textBaseline = "alphabetic", this.text = null, 
        this.width = null, this.height = null, this.pt2px = 1.28, $.extend(!0, this, options), 
        this.normalizedFontSize = this.normalizeFontSize(this.fontSize), this.setHeight();
    }, $.jqplot.CanvasTextRenderer.prototype.init = function(options) {
        $.extend(!0, this, options), this.normalizedFontSize = this.normalizeFontSize(this.fontSize), 
        this.setHeight();
    }, $.jqplot.CanvasTextRenderer.prototype.normalizeFontSize = function(sz) {
        sz = String(sz);
        var n = parseFloat(sz);
        return sz.indexOf("px") > -1 ? n / this.pt2px : sz.indexOf("pt") > -1 ? n : sz.indexOf("em") > -1 ? 12 * n : sz.indexOf("%") > -1 ? 12 * n / 100 : n / this.pt2px;
    }, $.jqplot.CanvasTextRenderer.prototype.fontWeight2Float = function(w) {
        if (Number(w)) return w / 400;
        switch (w) {
          case "normal":
            return 1;
        ...
        return this.text = t, this.setWidth(ctx), this;
    }, $.jqplot.CanvasTextRenderer.prototype.getWidth = function(ctx) {
        return this.width;
    }, $.jqplot.CanvasTextRenderer.prototype.setWidth = function(ctx, w) {
        return w ? this.width = w : this.width = this.measure(ctx, this.text), this;
    }, $.jqplot.CanvasTextRenderer.prototype.getHeight = function(ctx) {
        return this.height;
    }, $.jqplot.CanvasTextRenderer.prototype.setHeight = function(w) {
        return w ? this.height = w : this.height = this.normalizedFontSize * this.pt2px, 
        this;
        ...
        var i, c, total = 0, len = str.length;
        for (i = 0; len > i; i++) c = this.letter(str.charAt(i)), c && (total += c.width * this.normalizedFontSize / 25 * this.fontStretch);
        return total;
    }, $.jqplot.CanvasTextRenderer.prototype.draw = function(ctx, str) {
        var tx, ty, fact, i, j, c, penUp, needStroke, a, x = 0, y = .72 * this.height, total = 0, len = str.length, mag = this.normalizedFontSize / 25;
        for (ctx.save(), -Math.PI / 2 <= this.angle && this.angle <= 0 || 3 * Math.PI / 2 <= this.angle && this.angle <= 2 * Math.PI ? (tx = 0, 
        ty = -Math.sin(this.angle) * this.width) : 0 < this.angle && this.angle <= Math.PI / 2 || 2 * -Math.PI <= this.angle && this.angle <= 3 * -Math.PI / 2 ? (tx = Math.sin(this.angle) * this.height, 
        ty = 0) : -Math.PI < this.angle && this.angle < -Math.PI / 2 || Math.PI <= this.angle && this.angle <= 3 * Math.PI / 2 ? (tx = -Math.cos(this.angle) * this.width, 
        ty = -Math.sin(this.angle) * this.width - Math.cos(this.angle) * this.height) : (3 * -Math.PI / 2 < this.angle && this.angle < Math.PI || Math.PI / 2 < this.angle && this.angle < Math.PI) && (tx = Math.sin(this.angle) * this.height - Math.cos(this.angle) * this.width, 
        ty = -Math.cos(this.angle) * this.height), ctx.strokeStyle = this.fillStyle, ctx.fillStyle = this.fillStyle, 
        ctx.translate(tx, ty), ctx.rotate(this.angle), ctx.lineCap = "round", fact = this.normalizedFontSize > 30 ? 2 : 2 + (30 - this.normalizedFontSize) / 20, 
        ctx.lineWidth = fact * mag * this.fontWeight2Float(this.fontWeight), i = 0; len > i; i++) if (c = this.letter(str.charAt(i))) {
            for (ctx.beginPath(), penUp = 1, needStroke = 0, j = 0; j < c.points.length; j++) a = c.points[j], 
            -1 !== a[0] || -1 !== a[1] ? penUp ? (ctx.moveTo(x + a[0] * mag * this.fontStretch, y - a[1] * mag), 
            penUp = !1) : ctx.lineTo(x + a[0] * mag * this.fontStretch, y - a[1] * mag) : penUp = 1;
            ctx.stroke(), x += c.width * mag * this.fontStretch;
        }
        return ctx.restore(), total;
        ...
        options = options || {}, options.pt2px || (options.pt2px = 1.5), $.jqplot.CanvasTextRenderer.call(this, options);
    }, $.jqplot.CanvasFontRenderer.prototype = new $.jqplot.CanvasTextRenderer({}), 
    $.jqplot.CanvasFontRenderer.prototype.constructor = $.jqplot.CanvasFontRenderer, 
    $.jqplot.CanvasFontRenderer.prototype.measure = function(ctx, str) {
        var w, fstyle = this.fontSize + " " + this.fontFamily;
        return ctx.save(), ctx.font = fstyle, w = ctx.measureText(str).width, ctx.restore(), 
        w;
    }, $.jqplot.CanvasFontRenderer.prototype.draw = function(ctx, str) {
        var tx, ty, x = 0, y = .72 * this.height, fstyle = this.fontSize + " " + this.fontFamily;
        ctx.save(), -Math.PI / 2 <= this.angle && this.angle <= 0 || 3 * Math.PI / 2 <= this.angle && this.angle <= 2 * Math.PI ? (tx = 0, 
        ty = -Math.sin(this.angle) * this.width) : 0 < this.angle && this.angle <= Math.PI / 2 || 2 * -Math.PI <= this.angle && this.angle <= 3 * -Math.PI / 2 ? (tx = Math.sin(this.angle) * this.height, 
        ty = 0) : -Math.PI < this.angle && this.angle < -Math.PI / 2 || Math.PI <= this.angle && this.angle <= 3 * Math.PI / 2 ? (tx = -Math.cos(this.angle) * this.width, 
        ty = -Math.sin(this.angle) * this.width - Math.cos(this.angle) * this.height) : (3 * -Math.PI / 2 < this.angle && this.angle < Math.PI || Math.PI / 2 < this.angle && this.angle < Math.PI) && (tx = Math.sin(this.angle) * this.height - Math.cos(this.angle) * this.width, 
        ty = -Math.cos(this.angle) * this.height), ctx.strokeStyle = this.fillStyle, ctx.fillStyle = this.fillStyle, 
        ctx.font = fstyle, ctx.translate(tx, ty), ctx.rotate(this.angle), ctx.fillText(str, x, y), 
        ctx.restore();
    };
}(jQuery);
//# sourceMappingURL=jqplot.canvasTextRenderer.min.js.map